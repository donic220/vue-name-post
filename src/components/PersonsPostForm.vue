...
const color = computed(() => {
    return isValidName.value ? 'white': 'rgb(245,200,190)'
})
...
<template>
    <div class="form-container">
        <div class="input-container">
            ...
            <div v-if="!isValidName" class="error-message" > {{ nameLengthLimit }} characters or less, please</div>
            <div class="input-column">
                <span>age:</span>
                <input class="input" v-model="inputtingAge" type="number"/>
            </div>
        </div>
        <button :disabled="!isValidName" class="register-button" @click="onClickRegister">register</button>
    </div>
</template>
